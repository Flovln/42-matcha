...
class UserFeed extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.usersList.length > 0) {
      state['users'] = nextProps.usersList
      this.setState(state)
    }
  }
  ...
  render() {  
    ...
    return (
      <div className="user-feed">
        <h3>Here are some users who might interest you!</h3>
        <hr />
...
const mapStateToProps = state => {
  return {
    ...
    usersList: state.userInfos.usersMatchingListSuccess,
...
const mapDispatchToProps = dispatch => {
  return {
    loadUsersList: () => dispatch(loadMatchingUsers()),
